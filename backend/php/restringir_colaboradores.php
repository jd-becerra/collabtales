...
// Actualizar admite_colaboradores a 0
$stmt = $conn->prepare("UPDATE Cuento SET admite_colaboradores = 0 WHERE id_cuento = ?");
$stmt->bind_param("i", $id_cuento);
if (!$stmt->execute()) {
...
echo json_encode(["message" => "Colaboradores restringidos correctamente."]);

...
$sql_creador = $conn->prepare("SELECT fk_owner FROM Cuento WHERE id_cuento = ?");
$sql_creador->bind_param("i", $id_cuento);
$sql_creador->execute();
$creador_result = $sql_creador->get_result();
if ($creador_result->num_rows === 0) {
    http_response_code(404);
    echo json_encode(["error" => "Recurso no encontrado."]);
    exit;
}
$row_creador = $creador_result->fetch_assoc();
if ($row_creador['fk_owner'] != $id_alumno) {
    http_response_code(403);
    echo json_encode(["error" => "No tienes permiso para acceder a este recurso."]);
    exit;
}
